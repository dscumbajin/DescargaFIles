package com.ramostear.springboot.uploadfile;
...
public class CodeBase64 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        OutputStream out = new FileOutputStream("D:\\imagen.png");
        out.write(archivoByte);
        out.close();

        System.out.println("Inicia contuir zip");
        StringBuilder sb = new StringBuilder();
        sb.append("Test String");

        File f = new File("D:\\test.zip");
        ZipOutputStream outputStream = new ZipOutputStream(new FileOutputStream(f));
        ZipEntry e = new ZipEntry("imagen.png");
        outputStream.putNextEntry(e);

        outputStream.write(archivoByte, 0, archivoByte.length);
        outputStream.closeEntry();

        outputStream.close();
        System.out.println("Finaliza contuir zip");
    }
}
